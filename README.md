...
Adding messages (certification data) to a token. When you open a block chain file with a text editor that can display ASCII characters, your message will be visible. The message will be in ASCII table encoding accompanied by much data related to the management of the blockchain but unrelated to the message. This was done for the first time in the [bitcoin genesis block](https://en.bitcoin.it/wiki/Genesis_block) by storing the following sentence in the bitcoin block chain: "The Times 03/Jan/2009 Chancellor on brink of second bailout for banks". Examples [Raw block data](#MessageInAbitcoinRawData.JPG) [Broken block data](MessageInAbitcoinbrokenDownData.JPG).  
